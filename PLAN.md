...
- **Extension loaded and tested in a real Chrome profile: signup/login confirmed working end-to-end (verified via direct DB query); Gmail tracking injection was broken across two root causes, both fixed (ADR-12).** First fix (missing `scripting` permission) was necessary but not sufficient — the user still hit the same error afterward. Second fix (the `pageWorld.js` bundle + background-script message listener InboxSDK actually needs) was traced by reading the installed package source directly rather than continuing to guess. Both verified present in the built output (`grep`-confirmed, not just "build succeeded"). Not yet re-verified by an actual send in the browser — user needs to reload the extension and try again.
