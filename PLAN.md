...
- **Extension has not yet been loaded into a real Chrome profile or tested against a live Gmail account.** Backend is fully live and smoke-tested (see above), and the extension builds clean against real config — but loading `chrome://extensions` → unpacked, signing up (which requires clicking a real email confirmation link), and sending a test tracked email are steps only the account owner can complete. This is the last mile before the live-device acceptance test can run.
