...
- [x] Dashboard v2: open/click counts + first/last-opened timestamps per message (backend aggregation + UI), summary stats row, copyable message ID, visual redesign (badges, spacing, stat cards) — user-requested after seeing v1 live. Backend code shipped and tested but **not yet deployed** — blocked on a Cloudflare API token issue (see Known Issues), extension build ready and waiting.
...
- **Cloudflare API token now failing with error 9109 ("cannot use the access token from location: ...")** — appears to be an IP-restriction on the token that the current deploy environment's egress IP doesn't satisfy; worked for multiple earlier deploys in the same session, so something changed (either the token's configured allow-list, or the environment's outbound IP). No API/CLI path available to inspect or fix a token's own IP restrictions from the CLI. **Currently blocks:** deploying the dashboard v2 stats aggregation (code is written, tested, committed — just not live). User needs to check the token's IP Address Filtering setting in the Cloudflare dashboard (or issue a new unrestricted token) before the next deploy can proceed.
