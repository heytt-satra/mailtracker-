...
- **Extension loaded and tested in a real Chrome profile: signup/login confirmed working end-to-end (verified via direct DB query). InboxSDK itself now loads correctly (ADR-12, confirmed via the user's own error logs finally showing our extension ID/appId instead of "couldn't inject").** But the compose hook was STILL not creating messages after that fix — traced to ADR-13 (timeout too tight + fail-open catch block with zero logging, so the failure was real but completely invisible). Not yet re-verified by an actual send since ADR-13 shipped — user needs to reload the extension and try again; this time, if it still fails, the console will actually say why.
