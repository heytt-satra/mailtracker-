...
- **ADR-12: manifest needs the `scripting` permission for InboxSDK to function at all — missed in the original manifest, found live.** The user reported no tracking data reaching the backend at all; querying the live database directly (not guessing) confirmed signup/login/API-key issuance all worked, but the `messages` table was completely empty — meaning the compose hook's `POST /v1/messages` call never fired. The user's own Gmail console showed the real cause: `Error: Couldn't inject pageWorld.js. Check that the extension is using MV3 and has the correct permissions...` — InboxSDK's content script runs in an isolated JS world and needs to inject a bridge script into Gmail's actual page context via `chrome.scripting.executeScript({world: 'MAIN', ...})`, which requires the `scripting` permission. Verified against InboxSDK's own official reference manifest (`github.com/InboxSDK/hello-world`) before adding it, rather than guessing at a fix from the error text alone. This is a hole in Phase 2's InboxSDK research (ADR-3/ADR-9 verified the compose/conversations API surface but not the manifest-level bootstrapping requirements) that only surfaced once a real extension ran in a real Chrome profile — exactly the gap `wxt build` succeeding and 31 unit tests passing couldn't catch, since neither exercises real `chrome.*` permission enforcement.
...
- **Extension loaded and tested in a real Chrome profile: signup/login confirmed working end-to-end (verified via direct DB query); Gmail tracking injection was broken until ADR-12's `scripting` permission fix.** Root-caused via the user's own Gmail console errors rather than guessing. Not yet re-verified after the fix — user needs to reload the extension (`chrome://extensions` → reload icon) and send another test email.
...
- Diagnosed and fixed properly rather than pattern-matching a fix from the error string: fetched InboxSDK's own official reference manifest (`github.com/InboxSDK/hello-world`) and its content script to confirm exactly what was missing — a `"scripting"` permission, required because InboxSDK bridges into Gmail's actual page JS context (a separate world from the content script) via `chrome.scripting.executeScript({world: 'MAIN'})`, which Chrome blocks without that permission declared. Added it, verified it landed in the built `manifest.json` by reading the file directly rather than trusting the build log. Documented as ADR-12 — this was a real gap in the original InboxSDK research (Phase 2 verified the `Compose`/`Conversations` API surface thoroughly but never checked manifest-level bootstrapping requirements), and it's exactly the class of bug that 31 passing unit tests and a clean `wxt build` can't catch, since neither exercises real Chrome permission enforcement.
