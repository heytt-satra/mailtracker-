...
import { OUTLOOK_ASSETS, OUTLOOK_FUNCTIONS_HTML, OUTLOOK_MANIFEST_XML, OUTLOOK_TASKPANE_HTML } from './pages/outlook-addin';
...
/**
 * ADR-61 (Outlook add-in, C2). Serves the Outlook add-in's static assets —
 * generated by apps/outlook-addin/scripts/generate-worker-page.mjs after
 * `npm run build` in that workspace, embedded here the same way
 * landing.ts/privacy.ts embed HTML and beacon.ts's PIXEL_GIF_BASE64 embeds
 * binary data. Same origin as /v1/* on purpose — no CORS config needed for
 * this client (see PLAN.md). Sideload-testing only for now, not on
 * AppSource.
 */
app.get('/outlook/manifest.xml', (c) => {
  c.header('Content-Type', 'application/xml');
  return c.body(OUTLOOK_MANIFEST_XML);
});
app.get('/outlook/taskpane.html', (c) => c.html(OUTLOOK_TASKPANE_HTML));
app.get('/outlook/functions.html', (c) => c.html(OUTLOOK_FUNCTIONS_HTML));
app.get('/outlook/:path{.+}', (c) => {
  const asset = OUTLOOK_ASSETS[c.req.param('path')];
  if (!asset) return c.json({ error: 'Not found' }, 404);
  c.header('Content-Type', asset.contentType);
  if (asset.encoding === 'base64') {
    const bytes = Uint8Array.from(atob(asset.content), (ch) => ch.charCodeAt(0));
    return c.body(bytes);
  }
