...
const NO_SIGNAL: ReadSignal = { readConfidence: null, minEngagedSeconds: null, readEvidence: null };
...
export function computeReadSignal(events: VerdictEvent[]): ReadSignal {
  ...
  if (clicks.length > 0) {
    ...
    return { readConfidence: 'read', minEngagedSeconds: null, readEvidence: `Clicked a tracked link — genuine engagement confirmed` };
  }
  ...
  if (opens.length >= 2) {
    return {
      ...
      readEvidence: `Opened ${opens.length} separate times (first ${opens[0]}, last ${opens[opens.length - 1]}) — repeat engagement, but no single continuous duration is measurable`,
    };
  }
  ...
  if (opens.length === 1) {
    return {
      ...
      minEngagedSeconds: null,
      readEvidence: `Opened once — no further signal to confirm sustained reading`,
    };
  }
