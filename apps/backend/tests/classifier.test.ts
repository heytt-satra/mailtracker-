...
describe('regression: permanent fixtures (PLAN.md section 15)', () => {
  ...
  it('7. a GoogleImageProxy fetch shortly after send (e.g. 88s — a real observed timing) still escalates to opened once outside the 45s prefetch window', () => {
    // ADR-33 briefly gated this on a wider "Gmail auto-caching" window and
    // was reverted the same day: live data showed Gmail's own image-proxy
    // caching and a genuinely fast human open are indistinguishable by UA
    // and timing alone, and the wider window blocked the ordinary case of
    // opening an email within a few minutes — a worse failure than the
    // false positive it targeted. This fixture locks in the reverted,
    // previously-working behavior so it can't silently regress again.
    const result = classifyEvent(
      baseInput({
        event: baseEvent({ fetchSequenceMs: 88_000 }),
        isFirstFetch: false,
      }),
    );
    expect(result.verdict).toBe('verified_open');
    expect(nextStatus('delivered', result.verdict)).toBe('opened');
  });
});
